...
dir=$(dirname "$0")
# Must resolve exactly as state_dir() in snooze.py does. If the two ever
# disagreed, Python would write the state file in one place while this looked
# in another — and every tick would exit here, silently, forever.
state="${SNOOZE_STATE_DIR:-${HERDR_PLUGIN_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/herdr/plugins/${HERDR_PLUGIN_ID:-herdr-snooze}}}"

if [ "$1" = "tick" ] && [ ! -f "$state/snoozed.json" ]; then
  exit 0
fi
